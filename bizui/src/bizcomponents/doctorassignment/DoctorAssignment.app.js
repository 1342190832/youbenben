...
import styles from './DoctorAssignment.app.less'
...
class DoctorAssignmentBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
	  ...
	  <Menu
       ...
       >
             ...
             <Menu.Item key="dashboard">
               <Link to={`/doctorAssignment/${this.props.doctorAssignment.id}/dashboard`}><Icon type="dashboard" style={{marginRight:"20px"}}/><span>{appLocaleName(userContext,"Dashboard")}</span></Link>
             </Menu.Item>
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._doctorAssignment.selectedRows,
      role: "cq",
      currentUpdateIndex: state._doctorAssignment.currentUpdateIndex,
      owner: { type: '_doctorAssignment', id: state._doctorAssignment.id, listName: 'nolist', ref:state._doctorAssignment, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.DoctorAssignmentBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.doctorAssignment,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.doctorAssignment,"inline","dark")}
