...
import styles from './SupplyOrderConfirmation.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'supplyOrderConfirmation') , sorter: true },
...
const renderItemOfList=(supplyOrderConfirmation, targetComponent, columCount, listName)=>{
  
  if(!supplyOrderConfirmation){
  	return null
  }
  if(!supplyOrderConfirmation.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${supplyOrderConfirmation.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(supplyOrderConfirmation.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={supplyOrderConfirmation.id} size="small" col={displayColumnsCount} >
...
const SupplyOrderConfirmationBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
