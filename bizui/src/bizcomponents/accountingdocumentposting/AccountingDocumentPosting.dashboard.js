...
<Description term="ID" style={{wordBreak: 'break-all'}}>{accountingDocumentPosting.id}</Description> 
...
class AccountingDocumentPostingDashboard extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('accounting_document_posting'),cardsFor: "accountingDocumentPosting",
    	cardsSource: this.props.accountingDocumentPosting,returnURL,displayName,
  		subItems: [
{name: 'accountingDocumentList', displayName: window.mtrans('accounting_document','accounting_document_posting.accounting_document_list',false) ,viewGroup:'__no_group', type:'accountingDocument',count:accountingDocumentCount,addFunction: true, role: 'accountingDocument', metaInfo: accountingDocumentListMetaInfo, renderItem: GlobalComponents.AccountingDocumentBase.renderItemOfList},
