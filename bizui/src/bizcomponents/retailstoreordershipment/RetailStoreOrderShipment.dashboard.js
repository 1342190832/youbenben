...
<Description term="ID" style={{wordBreak: 'break-all'}}>{retailStoreOrderShipment.id}</Description> 
...
class RetailStoreOrderShipmentDashboard extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('retail_store_order_shipment'),cardsFor: "retailStoreOrderShipment",
    	cardsSource: this.props.retailStoreOrderShipment,returnURL,displayName,
  		subItems: [
{name: 'retailStoreOrderList', displayName: window.mtrans('retail_store_order','retail_store_order_shipment.retail_store_order_list',false) ,viewGroup:'__no_group', type:'retailStoreOrder',count:retailStoreOrderCount,addFunction: true, role: 'retailStoreOrder', metaInfo: retailStoreOrderListMetaInfo, renderItem: GlobalComponents.RetailStoreOrderBase.renderItemOfList},
