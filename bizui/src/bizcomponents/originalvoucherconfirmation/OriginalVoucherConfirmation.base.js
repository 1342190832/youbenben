...
import styles from './OriginalVoucherConfirmation.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'originalVoucherConfirmation') , sorter: true },
...
const renderItemOfList=(originalVoucherConfirmation, targetComponent, columCount, listName)=>{
  
  if(!originalVoucherConfirmation){
  	return null
  }
  if(!originalVoucherConfirmation.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${originalVoucherConfirmation.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(originalVoucherConfirmation.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={originalVoucherConfirmation.id} size="small" col={displayColumnsCount} >
...
const OriginalVoucherConfirmationBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
