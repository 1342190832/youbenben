...
import styles from './EmployeeBoarding.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'employeeBoarding') , sorter: true },
...
const renderItemOfList=(employeeBoarding, targetComponent, columCount, listName)=>{
  
  if(!employeeBoarding){
  	return null
  }
  if(!employeeBoarding.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${employeeBoarding.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(employeeBoarding.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={employeeBoarding.id} size="small" col={displayColumnsCount} >
...
const packFormValuesToObject = ( formValuesToPack )=>{
	...
	const data = {who, employTime:moment(employTime).valueOf(), comments}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {who, employTime, comments} = objectToUnpack

	const data = {who, employTime:moment(employTime), comments}
	return data
}
...
const EmployeeBoardingBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
