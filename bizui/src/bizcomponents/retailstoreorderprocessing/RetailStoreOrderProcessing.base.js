...
import styles from './RetailStoreOrderProcessing.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'retailStoreOrderProcessing') , sorter: true },
...
const renderItemOfList=(retailStoreOrderProcessing, targetComponent, columCount, listName)=>{
  
  if(!retailStoreOrderProcessing){
  	return null
  }
  if(!retailStoreOrderProcessing.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${retailStoreOrderProcessing.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(retailStoreOrderProcessing.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={retailStoreOrderProcessing.id} size="small" col={displayColumnsCount} >
...
const packFormValuesToObject = ( formValuesToPack )=>{
	...
	const data = {who, processTime:moment(processTime).valueOf()}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {who, processTime} = objectToUnpack

	const data = {who, processTime:moment(processTime)}
	return data
}
...
const RetailStoreOrderProcessingBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
