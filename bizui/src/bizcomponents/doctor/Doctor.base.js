...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'doctor') , sorter: true },
...
const renderItemOfList=(doctor, targetComponent, columCount, listName)=>{
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${doctor.id}`} style={{marginTop:"10px"}}>
	...
	<Col span={20}>
      ...
      <DescriptionList  key={doctor.id} size="small" col={displayColumnsCount} >
        ...
        <Description term={fieldLabels.shotImage}><div><ImagePreview imageTitle={fieldLabels.shotImage} imageLocation={doctor.shotImage}/></div></Description> 
