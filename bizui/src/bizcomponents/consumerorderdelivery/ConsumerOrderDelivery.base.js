...
import styles from './ConsumerOrderDelivery.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'consumerOrderDelivery') , sorter: true },
...
const renderItemOfList=(consumerOrderDelivery, targetComponent, columCount, listName)=>{
  
  if(!consumerOrderDelivery){
  	return null
  }
  if(!consumerOrderDelivery.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${consumerOrderDelivery.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(consumerOrderDelivery.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={consumerOrderDelivery.id} size="small" col={displayColumnsCount} >
...
const ConsumerOrderDeliveryBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
