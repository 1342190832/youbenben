...
import styles from './AccountingDocumentCreation.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'accountingDocumentCreation') , sorter: true },
...
const renderItemOfList=(accountingDocumentCreation, targetComponent, columCount, listName)=>{
  
  if(!accountingDocumentCreation){
  	return null
  }
  if(!accountingDocumentCreation.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${accountingDocumentCreation.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(accountingDocumentCreation.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={accountingDocumentCreation.id} size="small" col={displayColumnsCount} >
...
const AccountingDocumentCreationBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
