...
import styles from './ExpenseItem.app.less'
...
class ExpenseItemBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
	  ...
	  <Menu
       ...
       >
             ...
             <Menu.Item key="dashboard">
               <Link to={`/expenseItem/${this.props.expenseItem.id}/dashboard`}><Icon type="dashboard" style={{marginRight:"20px"}}/><span>{appLocaleName(userContext,"Dashboard")}</span></Link>
             </Menu.Item>
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._expenseItem.selectedRows,
      role: "cq",
      currentUpdateIndex: state._expenseItem.currentUpdateIndex,
      owner: { type: '_expenseItem', id: state._expenseItem.id, listName: 'nolist', ref:state._expenseItem, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
  	const {ExpenseItemDashboard} = GlobalComponents
  	const {ExpenseItemPermission} = GlobalComponents
  	const {ExpenseItemProfile} = GlobalComponents
  	
  	
  	const routers=[
  	{path:"/expenseItem/:id/dashboard", component: ExpenseItemDashboard},
  	{path:"/expenseItem/:id/profile", component: ExpenseItemProfile},
  	{path:"/expenseItem/:id/permission", component: ExpenseItemPermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.ExpenseItemBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.expenseItem,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.expenseItem,"inline","dark")}
