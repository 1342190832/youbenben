...
import styles from './OriginalVoucherCreation.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'originalVoucherCreation') , sorter: true },
...
const renderItemOfList=(originalVoucherCreation, targetComponent, columCount, listName)=>{
  
  if(!originalVoucherCreation){
  	return null
  }
  if(!originalVoucherCreation.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${originalVoucherCreation.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(originalVoucherCreation.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={originalVoucherCreation.id} size="small" col={displayColumnsCount} >
...
const OriginalVoucherCreationBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
