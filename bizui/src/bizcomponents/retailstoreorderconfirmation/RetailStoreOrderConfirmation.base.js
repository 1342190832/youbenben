...
import styles from './RetailStoreOrderConfirmation.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'retailStoreOrderConfirmation') , sorter: true },
...
const renderItemOfList=(retailStoreOrderConfirmation, targetComponent, columCount, listName)=>{
  
  if(!retailStoreOrderConfirmation){
  	return null
  }
  if(!retailStoreOrderConfirmation.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${retailStoreOrderConfirmation.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(retailStoreOrderConfirmation.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={retailStoreOrderConfirmation.id} size="small" col={displayColumnsCount} >
...
const packFormValuesToObject = ( formValuesToPack )=>{
	...
	const data = {who, confirmTime:moment(confirmTime).valueOf()}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {who, confirmTime} = objectToUnpack

	const data = {who, confirmTime:moment(confirmTime)}
	return data
}
...
const RetailStoreOrderConfirmationBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
