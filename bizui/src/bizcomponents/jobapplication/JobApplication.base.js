...
import styles from './JobApplication.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'jobApplication') , sorter: true },
...
const renderItemOfList=(jobApplication, targetComponent, columCount, listName)=>{
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${jobApplication.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(jobApplication.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={jobApplication.id} size="small" col={displayColumnsCount} >
...
const packFormValuesToObject = ( formValuesToPack )=>{
	...
	const data = {applicationTime:moment(applicationTime).valueOf(), who, comments}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {applicationTime, who, comments} = objectToUnpack

	const data = {applicationTime:moment(applicationTime), who, comments}
	return data
}
...
const JobApplicationBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
