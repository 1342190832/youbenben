...
import styles from './RetailStoreOrderDelivery.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'retailStoreOrderDelivery') , sorter: true },
...
const renderItemOfList=(retailStoreOrderDelivery, targetComponent, columCount, listName)=>{
  
  if(!retailStoreOrderDelivery){
  	return null
  }
  if(!retailStoreOrderDelivery.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${retailStoreOrderDelivery.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(retailStoreOrderDelivery.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={retailStoreOrderDelivery.id} size="small" col={displayColumnsCount} >
...
const packFormValuesToObject = ( formValuesToPack )=>{
	...
	const data = {who, deliveryTime:moment(deliveryTime).valueOf()}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {who, deliveryTime} = objectToUnpack

	const data = {who, deliveryTime:moment(deliveryTime)}
	return data
}
...
const RetailStoreOrderDeliveryBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
