...
import styles from './ConsumerOrderConfirmation.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'consumerOrderConfirmation') , sorter: true },
...
const renderItemOfList=(consumerOrderConfirmation, targetComponent, columCount, listName)=>{
  
  if(!consumerOrderConfirmation){
  	return null
  }
  if(!consumerOrderConfirmation.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${consumerOrderConfirmation.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(consumerOrderConfirmation.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={consumerOrderConfirmation.id} size="small" col={displayColumnsCount} >
...
const ConsumerOrderConfirmationBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
