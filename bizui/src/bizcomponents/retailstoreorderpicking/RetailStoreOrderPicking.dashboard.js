...
<Description term="ID" style={{wordBreak: 'break-all'}}>{retailStoreOrderPicking.id}</Description> 
<Description term="谁" style={{wordBreak: 'break-all'}}>{retailStoreOrderPicking.who}</Description> 
<Description term="处理时间">{ moment(retailStoreOrderPicking.processTime).format('YYYY-MM-DD')}</Description> 
...
class RetailStoreOrderPickingDashboard extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('retail_store_order_picking'),cardsFor: "retailStoreOrderPicking",
    	cardsSource: this.props.retailStoreOrderPicking,returnURL,displayName,
  		subItems: [
{name: 'retailStoreOrderList', displayName: window.mtrans('retail_store_order','retail_store_order_picking.retail_store_order_list',false) ,viewGroup:'__no_group', type:'retailStoreOrder',count:retailStoreOrderCount,addFunction: true, role: 'retailStoreOrder', metaInfo: retailStoreOrderListMetaInfo, renderItem: GlobalComponents.RetailStoreOrderBase.renderItemOfList},
