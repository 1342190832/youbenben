...
<Description term="ID" style={{wordBreak: 'break-all'}}>{supplyOrderProcessing.id}</Description> 
<Description term="谁" style={{wordBreak: 'break-all'}}>{supplyOrderProcessing.who}</Description> 
<Description term="处理时间">{ moment(supplyOrderProcessing.processTime).format('YYYY-MM-DD')}</Description> 
...
class SupplyOrderProcessingDashboard extends Component {
 ...
 state = {
  ...
  render() {
    ...
    const cardsData = {cardsName:window.trans('supply_order_processing'),cardsFor: "supplyOrderProcessing",
    	cardsSource: this.props.supplyOrderProcessing,returnURL,displayName,
  		subItems: [
{name: 'consumerOrderList', displayName: window.mtrans('consumer_order','supply_order_processing.consumer_order_list',false) ,viewGroup:'__no_group', type:'consumerOrder',count:consumerOrderCount,addFunction: true, role: 'consumerOrder', metaInfo: consumerOrderListMetaInfo, renderItem: GlobalComponents.ConsumerOrderBase.renderItemOfList},
{name: 'supplyOrderList', displayName: window.mtrans('supply_order','supply_order_processing.supply_order_list',false) ,viewGroup:'__no_group', type:'supplyOrder',count:supplyOrderCount,addFunction: true, role: 'supplyOrder', metaInfo: supplyOrderListMetaInfo, renderItem: GlobalComponents.SupplyOrderBase.renderItemOfList},
