...
import styles from './SupplyOrderProcessing.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'supplyOrderProcessing') , sorter: true },
...
const renderItemOfList=(supplyOrderProcessing, targetComponent, columCount, listName)=>{
  
  if(!supplyOrderProcessing){
  	return null
  }
  if(!supplyOrderProcessing.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${supplyOrderProcessing.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(supplyOrderProcessing.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={supplyOrderProcessing.id} size="small" col={displayColumnsCount} >
...
const SupplyOrderProcessingBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
