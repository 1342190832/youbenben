...
import styles from './SupplyOrderPicking.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'supplyOrderPicking') , sorter: true },
...
const renderItemOfList=(supplyOrderPicking, targetComponent, columCount, listName)=>{
  
  if(!supplyOrderPicking){
  	return null
  }
  if(!supplyOrderPicking.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${supplyOrderPicking.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(supplyOrderPicking.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={supplyOrderPicking.id} size="small" col={displayColumnsCount} >
...
const SupplyOrderPickingBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
