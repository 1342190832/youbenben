...
import styles from './OriginalVoucherAuditing.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'originalVoucherAuditing') , sorter: true },
...
const renderItemOfList=(originalVoucherAuditing, targetComponent, columCount, listName)=>{
  
  if(!originalVoucherAuditing){
  	return null
  }
  if(!originalVoucherAuditing.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${originalVoucherAuditing.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(originalVoucherAuditing.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={originalVoucherAuditing.id} size="small" col={displayColumnsCount} >
...
const OriginalVoucherAuditingBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
