...
import styles from './ConsumerOrderProcessing.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'consumerOrderProcessing') , sorter: true },
...
const renderItemOfList=(consumerOrderProcessing, targetComponent, columCount, listName)=>{
  
  if(!consumerOrderProcessing){
  	return null
  }
  if(!consumerOrderProcessing.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${consumerOrderProcessing.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(consumerOrderProcessing.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={consumerOrderProcessing.id} size="small" col={displayColumnsCount} >
...
const ConsumerOrderProcessingBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
