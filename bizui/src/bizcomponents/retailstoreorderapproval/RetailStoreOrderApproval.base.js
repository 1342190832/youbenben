...
import styles from './RetailStoreOrderApproval.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'retailStoreOrderApproval') , sorter: true },
...
const renderItemOfList=(retailStoreOrderApproval, targetComponent, columCount, listName)=>{
  
  if(!retailStoreOrderApproval){
  	return null
  }
  if(!retailStoreOrderApproval.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${retailStoreOrderApproval.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(retailStoreOrderApproval.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={retailStoreOrderApproval.id} size="small" col={displayColumnsCount} >
...
const packFormValuesToObject = ( formValuesToPack )=>{
	...
	const data = {who, approveTime:moment(approveTime).valueOf()}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {who, approveTime} = objectToUnpack

	const data = {who, approveTime:moment(approveTime)}
	return data
}
...
const RetailStoreOrderApprovalBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
