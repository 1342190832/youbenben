...
import styles from './OfferAcceptance.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'offerAcceptance') , sorter: true },
...
const renderItemOfList=(offerAcceptance, targetComponent, columCount, listName)=>{
  
  if(!offerAcceptance){
  	return null
  }
  if(!offerAcceptance.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${offerAcceptance.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(offerAcceptance.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={offerAcceptance.id} size="small" col={displayColumnsCount} >
...
const packFormValuesToObject = ( formValuesToPack )=>{
	...
	const data = {who, acceptTime:moment(acceptTime).valueOf(), comments}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {who, acceptTime, comments} = objectToUnpack

	const data = {who, acceptTime:moment(acceptTime), comments}
	return data
}
...
const OfferAcceptanceBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
