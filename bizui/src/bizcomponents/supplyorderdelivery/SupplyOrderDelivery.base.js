...
import styles from './SupplyOrderDelivery.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'supplyOrderDelivery') , sorter: true },
...
const renderItemOfList=(supplyOrderDelivery, targetComponent, columCount, listName)=>{
  
  if(!supplyOrderDelivery){
  	return null
  }
  if(!supplyOrderDelivery.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${supplyOrderDelivery.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(supplyOrderDelivery.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={supplyOrderDelivery.id} size="small" col={displayColumnsCount} >
...
const SupplyOrderDeliveryBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
