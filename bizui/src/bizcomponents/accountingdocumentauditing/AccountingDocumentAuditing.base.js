...
import styles from './AccountingDocumentAuditing.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'accountingDocumentAuditing') , sorter: true },
...
const renderItemOfList=(accountingDocumentAuditing, targetComponent, columCount, listName)=>{
  
  if(!accountingDocumentAuditing){
  	return null
  }
  if(!accountingDocumentAuditing.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${accountingDocumentAuditing.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(accountingDocumentAuditing.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={accountingDocumentAuditing.id} size="small" col={displayColumnsCount} >
...
const AccountingDocumentAuditingBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
