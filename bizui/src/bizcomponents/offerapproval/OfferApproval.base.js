...
import styles from './OfferApproval.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'offerApproval') , sorter: true },
...
const renderItemOfList=(offerApproval, targetComponent, columCount, listName)=>{
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${offerApproval.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(offerApproval.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={offerApproval.id} size="small" col={displayColumnsCount} >
...
const packFormValuesToObject = ( formValuesToPack )=>{
	...
	const data = {who, approveTime:moment(approveTime).valueOf(), comments}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {who, approveTime, comments} = objectToUnpack

	const data = {who, approveTime:moment(approveTime), comments}
	return data
}
...
const OfferApprovalBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
