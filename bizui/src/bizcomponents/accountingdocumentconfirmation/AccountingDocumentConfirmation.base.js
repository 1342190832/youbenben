...
import styles from './AccountingDocumentConfirmation.base.less'
const {
...
const displayColumns = [
  { title: fieldLabels.id, debugtype: 'string', dataIndex: 'id', width: '6', render: (text, record)=>renderTextCell(text,record,'accountingDocumentConfirmation') , sorter: true },
...
const renderItemOfList=(accountingDocumentConfirmation, targetComponent, columCount, listName)=>{
  
  if(!accountingDocumentConfirmation){
  	return null
  }
  if(!accountingDocumentConfirmation.id){
  	return null
  }
  ...
  const userContext = null
  return (
    <Card key={`${listName}-${accountingDocumentConfirmation.id}`} style={{marginTop:"10px"}}>
		
	<Col span={4}>
		<Avatar size={90} style={{ backgroundColor: genColor(), verticalAlign: 'middle' }}>
			{leftChars(accountingDocumentConfirmation.displayName)}
		</Avatar>
	</Col>
	<Col span={20}>
      ...
      <DescriptionList  key={accountingDocumentConfirmation.id} size="small" col={displayColumnsCount} >
...
const packFormValuesToObject = ( formValuesToPack )=>{
	...
	const data = {who, comments, makeDate:moment(makeDate).valueOf()}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {who, comments, makeDate} = objectToUnpack

	const data = {who, comments, makeDate:moment(makeDate)}
	return data
}
...
const AccountingDocumentConfirmationBase={menuData,settingMenuData,displayColumns,fieldLabels,renderItemOfList, stepOf, searchLocalData}
