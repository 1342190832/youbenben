...
import styles from './DoctorSchedule.app.less'
...
class DoctorScheduleBizApp extends React.PureComponent {
constructor(props) {
    super(props)
     this.state = {
 ...
 getNavMenuItems = (targetObject, style, customTheme) => {
	  ...
	  <Menu
       ...
       >
             ...
             <Menu.Item key="dashboard">
               <Link to={`/doctorSchedule/${this.props.doctorSchedule.id}/dashboard`}><Icon type="dashboard" style={{marginRight:"20px"}}/><span>{appLocaleName(userContext,"Dashboard")}</span></Link>
             </Menu.Item>
  ...
  getRequestTypeStepForm = () => {
    ...
    return connect(state => ({
      selectedRows: state._doctorSchedule.selectedRows,
      role: "cq",
      currentUpdateIndex: state._doctorSchedule.currentUpdateIndex,
      owner: { type: '_doctorSchedule', id: state._doctorSchedule.id, listName: 'nolist', ref:state._doctorSchedule, listDisplayName: appLocaleName(userContext,"List") }, // this is for model namespace and
  ...
  buildRouters = () =>{
  	const {DoctorScheduleDashboard} = GlobalComponents
  	const {DoctorSchedulePermission} = GlobalComponents
  	const {DoctorScheduleProfile} = GlobalComponents
  	
  	
  	const routers=[
  	{path:"/doctorSchedule/:id/dashboard", component: DoctorScheduleDashboard},
  	{path:"/doctorSchedule/:id/profile", component: DoctorScheduleProfile},
  	{path:"/doctorSchedule/:id/permission", component: DoctorSchedulePermission},
  ...
  handleOpenChange = (openKeys) => {
   ...
   render() {
    ...
    const {searchLocalData}=GlobalComponents.DoctorScheduleBase
    ...
    const renderMenuSwitch=()=>{
     ...
     <Layout>
      ...
      {this.state.showSearch&&(
        ...
        <div style={{backgroundColor:'black'}}  onClick={()=>hideSearchResult()}  >{searchLocalData(this.props.doctorSchedule,this.state.searchKeyword)}</div>
       ...
       <Sider
        ...
        >
         ...
         {this.getNavMenuItems(this.props.doctorSchedule,"inline","dark")}
