...
public class CRFieldData {
	protected String name;	//: person_info.male.name          // 前台直接使用，不需要解析。 后台按照<scene_code>.<group>.<field name> 拼装
	...
	protected String type;	//: text  // 可为 text,long_text,money,integer,decimal,images,document,date,datetime,time,boolean, prompt_message(表示这个仅是一段文本)
	...
	protected boolean multiple=false;	//: true => 只有candidateValues有值才有效。 单选的，value就是type表示的类型；多选的，value是json数组。
	protected String candidateValuesApi;	//: wxappService/getCandidateCity/:grp_1.male.province/  => 只有级联变动的字段才有
