...
public abstract class BaseChangeRequestHelper {
	...
	public static CRSceneSpec SCENE(CRSpec crSpec, String sceneCode) {
		CRSceneSpec sceneSpec = crSpec.getSceneList().stream().filter(scene -> scene.getName().equals(sceneCode))
	...
	protected void setFieldSpecInfo(CRGroupData groupData, CRFieldData fieldData, CRFieldSpec fieldSpec) {
	...
	protected Object makeFieldCandidateValues(CRGroupData groupData, CRFieldData fieldData, CRFieldSpec fieldSpec) {
	...
	protected Object convertToUiCandidateValues(CRFieldSpec fieldSpec, Object candidateValues) {
		...
		if (candidateValues instanceof List) {
			...
			return list.stream()
					.map(it -> MapUtil.put("title", getCandidateDataTitle(fieldSpec, it)).put("value", getCandidateDataValue(fieldSpec, it)).into_map())
					.collect(Collectors.toList());
	...
	protected void addRegExpRule(List<Map<String, Object>> rules, CRFieldSpec fieldSpec) {
		...
		rules.add(MapUtil.put("regex", fieldSpec.getRegExp()).put("message", "请输入正确格式的"+fieldSpec.getLabel()).into_map());
	}
	...
	protected void addRangeRule(List<Map<String, Object>> rules, CRFieldSpec fieldSpec) {
		...
		if (fieldSpec.getType().equals("image")) {
			Map<String, Object> imageRangeRule = MapUtil.put("min", fieldSpec.getRequired()?1:0)
					.put("max", 1)
					.put("type", "array")
		...
		if (maxStr != null && minStr != null) {
			Map<String, Object> rule = MapUtil.put("min", minStr)
					.put("max", maxStr)
					.put("type", params.get("type"))
					.put("message", String.format("%s%s不能%s%s%s,也不能%s%s%s", fieldSpec.getLabel(), 
							params.get("dim"), params.get("lowWord"), minStr, params.get("unit"),
									params.get("hiWord"), maxStr, params.get("unit"))).into_map();
			rules.add(rule);
			return;
		}
		
		if (maxStr != null) {
			Map<String, Object> rule = MapUtil
					.put("max", maxStr)
					.put("type", params.get("type"))
					.put("message", String.format("%s%s不能%s%s%s", fieldSpec.getLabel(), 
							params.get("dim"), params.get("hiWord"), maxStr, params.get("unit"))).into_map();
			rules.add(rule);
			return;
		}
		...
		Map<String, Object> rule = MapUtil.put("min", minStr)
				.put("type", params.get("type"))
				.put("message", String.format("%s%s不能%s%s%s", fieldSpec.getLabel(), 
						params.get("dim"), params.get("lowWord"), minStr, params.get("unit"))).into_map();
		rules.add(rule);
	}
	...
	protected void addRequiredRule(List<Map<String, Object>> rules, CRFieldSpec fieldSpec) {
		...
		rules.add(MapUtil.put("required", true).put("message", fieldSpec.getLabel()+"不能为空").into_map());
	}
	...
	protected void fulfillChangeRequestActions(ChangeRequestData reuestData, CRSpec crSpec, String sceneCode) {
		if (crSpec.isOnlyOneScence(sceneCode)) {
			addCommitAction(reuestData, crSpec);
			return;
		}
		if (crSpec.isFirstScence(sceneCode)) {
			addNextStepAction(reuestData, crSpec);
			return;
		}
		if (crSpec.isLastScence(sceneCode)) {
			addPrevStepAction(reuestData, crSpec);
			addCommitAction(reuestData, crSpec);
			return;
		}
		addPrevStepAction(reuestData, crSpec);
	...
	protected void addNextStepAction(ChangeRequestData reuestData, CRSpec crSpec) {
		reuestData.addAction(crSpec, BaseCrConst.ACTION_NEXT_STEP, "下一步",
				makeActionUrl(crSpec.getRequestPrefix(), crSpec.getChangeRequestType(), "nextStep", "", 0));
	}

	protected void addCommitAction(ChangeRequestData reuestData, CRSpec crSpec) {
		reuestData.addAction(crSpec, BaseCrConst.ACTION_COMMIT, "提交",
				makeActionUrl(crSpec.getRequestPrefix(), crSpec.getChangeRequestType(), "commit", "", 0));
	}

	protected void addPrevStepAction(ChangeRequestData reuestData, CRSpec crSpec) {
		reuestData.addAction(crSpec, BaseCrConst.ACTION_PREV_STEP, "上一步",
				makeActionUrl(crSpec.getRequestPrefix(), crSpec.getChangeRequestType(), "prevStep", "", 0));
	}

	protected CRFieldSpec findCorrespondingSpec(List<CRFieldSpec> fieldSpecList, String fieldName) {
		return fieldSpecList.stream().filter(fs -> fieldName.startsWith(fs.getFieldGroup() + "_")).findFirst()
				.orElse(null);
	}
	...
	protected String makeActionUrl(String prefix, String crType, String actionCode, String groupName, int recordIndex) {
		StringBuilder sb = new StringBuilder();
		try {
			sb.append(prefix)// .append("x/")
					.append("?").append("t=").append(URLEncoder.encode(crType, "UTF-8")).append("&").append("a=")
					.append(URLEncoder.encode(actionCode, "UTF-8")).append("&").append("g=")
					.append(URLEncoder.encode(groupName, "UTF-8")).append("&").append("i=")
	...
	public ChangeRequestPostData parsePostedData(Map<String, Object> requestParameters) throws Exception {
		...
		Map<String, Object> postedFormData = DebugUtil.getObjectMapper().readValue(postedFormDataStr, Map.class);
		// System.out.println(DebugUtil.dumpAsJson(postedFormData, true));
		String sceneCode = RequestUtil.getStringInput(postedFormData,
				BaseCrConst.GROUP_HIDDEN + "_" + BaseCrConst.FIELD_SCENE_CODE, null);
		String crId = RequestUtil.getStringInput(postedFormData,
				BaseCrConst.GROUP_HIDDEN + "_" + BaseCrConst.FIELD_CR_ID, null);
		...
		groupSpecList.forEach(groupSpec -> {
		...
		for (CRGroupSpec groupSpec : groupSpecList) {
			...
			List<CRFieldSpec> fieldSpecList = crSpec.getFieldList().stream()
					.filter(f -> GROUP_NAME(f).equals(groupName)).collect(Collectors.toList());
			for (CRFieldSpec fieldSpec : fieldSpecList) {
				if (fieldSpec.getInteractionMode().equals("display")) {
					continue;
				}

				String fieldName = postedFormData.keySet().stream()
						.filter(it -> it.startsWith(fieldSpec.getName() + "_")).findFirst().orElse(null);
	...
	protected abstract void doFieldChecking(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,	Object fieldValue);
	protected abstract Object queryCandidatesForModel(CRGroupData groupData, CRFieldData fieldData, CRFieldSpec fieldSpec);
	...
	protected void doStringFieldChecking(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,
			Object value) {
		String val = getStringValue(value);
		if (val == null) {
			postedData.addVerifyErrorMessage(fieldName, fieldSpec.getLabel(),
					String.format("%s的输入不正确", fieldSpec.getLabel()));
			return;
		}
	...
	protected void doDatetimeFieldChecking(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,
			Object value) {
	...
	protected void doLongtextFieldChecking(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,
			Object value) {
		String val = getStringValue(value);
		if (val == null) {
			postedData.addVerifyErrorMessage(fieldName, fieldSpec.getLabel(),
	...
	protected void doArticleFieldChecking(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,
			Object value) {
		doLongtextFieldChecking(postedData, fieldSpec, fieldName, value);
	}
	
	protected void doBooleanFieldChecking(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,
			Object value) {
	...
	protected void doTimeFieldChecking(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,
			Object value) {
	...
	protected void doIntegerFieldChecking(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,
			Object value) {
	...
	protected void doMoneyFieldChecking(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,
			Object value) {
	...
	protected void doDecimalFieldChecking(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,
			Object value) {
	...
	protected void doImagesFieldChecking(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,
			Object value) {
	...
	protected void doDateFieldChecking(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,
			Object value) {
		Date val = this.getDatetimeValue(value);
		if (val == null) {
			postedData.addVerifyErrorMessage(fieldName, fieldSpec.getLabel(),
	...
	protected void doImageFieldChecking(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,
			Object value) {
	...
	protected String calcSceneCode(ChangeRequestPostData postedData) throws Exception {
		...
		CRSpec crSpec = CR(crType);
		CRSceneSpec sceneSpec = crSpec.getSceneList().stream().filter(it -> it.getName().equals(sceneCode)).findFirst()
				.orElse(null);
		if (sceneSpec == null) {
		...
		default:
			error("您的请求有异常,目前不支持" + crSpec.getTitle() + "." + sceneSpec.getTitle() + "的" + actionCode + "操作");
		}
		return null;
	}

	protected boolean isFieldMissing(ChangeRequestPostData postedData, CRFieldSpec fieldSpec, String fieldName,
			Object value) {
		if (value != null) {
			return false;
		}
		...
		if (fieldSpec.getRequired()) {
			postedData.addVerifyErrorMessage(fieldName, fieldSpec.getLabel(),
					String.format("请输入'%s'", fieldSpec.getLabel()));
		}
		return true;
	}
	...
	protected String getImageValue(Object value) {
		...
		try {
			List<ImageInfo> imgList = DebugUtil.getObjectMapper().readValue(imgContent,
					new TypeReference<List<ImageInfo>>() {
					});
	...
	protected int getIndexOfGroup(ChangeRequestData reuestData, CRGroupSpec groupSpec) {
		...
		CRFieldData f = HIDDEN_GROUP(reuestData).getFieldList().stream().filter(it -> it.getName().equals(gName))
				.findFirst().orElse(null);
		if (f == null) {
			return 1;
		}
		return Math.max(1, Math.min(Integer.valueOf((String)f.getValue()), groupSpec.getMaximumNumber()));
	}

	protected void fulfillGroupActions(CRSpec crSpec, CRGroupData groupData, CRGroupSpec groupSpec,
		...
		if (groupRecordIndex > 1) {
			this.addPrevRecordAction(groupData, crSpec, groupRecordIndex);
		}
		if (!groupData.getFieldList().get(0).getName().endsWith("_new")) {
			this.addDeleteRecordAction(groupData, crSpec, groupRecordIndex);
		}
		if (groupRecordIndex < groupSpec.getMaximumNumber()) {
			this.addNextRecordAction(groupData, crSpec, groupRecordIndex);
		}
	}

	protected void addNextRecordAction(CRGroupData groupData, CRSpec crSpec, int groupRecordIndex) {
		groupData.addAction(crSpec, BaseCrConst.ACTION_NEXT_RECORD, "下一条", makeActionUrl(crSpec.getRequestPrefix(),
				crSpec.getChangeRequestType(), BaseCrConst.ACTION_NEXT_RECORD, groupData.getName(), groupRecordIndex));
	}

	protected void addPrevRecordAction(CRGroupData groupData, CRSpec crSpec, int groupRecordIndex) {
		groupData.addAction(crSpec, BaseCrConst.ACTION_PREV_RECORD, "上一条", makeActionUrl(crSpec.getRequestPrefix(),
				crSpec.getChangeRequestType(), BaseCrConst.ACTION_PREV_RECORD, groupData.getName(), groupRecordIndex));
	}

	protected void addDeleteRecordAction(CRGroupData groupData, CRSpec crSpec, int groupRecordIndex) {
		groupData.addAction(crSpec, BaseCrConst.ACTION_DELETE_RECORD, "删除",
				makeActionUrl(crSpec.getRequestPrefix(), crSpec.getChangeRequestType(),
						BaseCrConst.ACTION_DELETE_RECORD, groupData.getName(), groupRecordIndex));
	}

	protected Map<String, Integer> calcNewGroupIds(ChangeRequestPostData postedData) throws Exception {
	...
	protected List<Map<String, Object>> hiddenFieldRules(String fieldShortName) {
		return CollectionUtils.toList(MapUtil.put("required", true).put("message", fieldShortName+"不能为空").into_map());
	}
	...
	protected boolean mustFillSomeField(CRSpec crSpec, List<CRGroupSpec> groupSpecList) {
		...
		List<CRGroupSpec> gsList = groupSpecList.stream().filter(it->it.isMustHave()).collect(Collectors.toList());
		if (gsList == null || gsList.isEmpty()) {
			return false;
		}
		List<CRFieldSpec> fsList = crSpec.getFieldList().stream().filter(it->it.getRequired() != null && it.getRequired().booleanValue()).collect(Collectors.toList());
	...
	protected List<Object> prepareParamsForQueryCandidatesBySpec(CRGroupData groupData, CRFieldData fieldData, CRFieldSpec fieldSpec) {
	...
	protected Object getValueFromFieldData(CRGroupData groupData, CRFieldData fieldData, String fieldName) {
		String[] curName = fieldData.getName().split("_");
		if (curName == null || curName.length != 4) {
			return null;
		}
		String fieldKey = groupData.getName() + "_"+fieldName + "_" + curName[3];
		CRFieldData srcField = groupData.getFieldList().stream().filter(it->it.getName().equals(fieldKey)).findFirst().orElse(null);
		if (srcField == null) {
			return null;
		}
