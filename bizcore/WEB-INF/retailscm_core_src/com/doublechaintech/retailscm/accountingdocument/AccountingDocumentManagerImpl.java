 ...
 //--------------------------------------------------------------
	...
	*/
    ...
    protected void enhanceForListOfView(RetailscmUserContext userContext,SmartList<AccountingDocument> list) throws Exception {
		...
		List<AccountingDocumentCreation> creationList = RetailscmBaseUtils.collectReferencedObjectWithType(userContext, list, AccountingDocumentCreation.class);
		userContext.getDAOGroup().enhanceList(creationList, AccountingDocumentCreation.class);
		List<AccountingDocumentConfirmation> confirmationList = RetailscmBaseUtils.collectReferencedObjectWithType(userContext, list, AccountingDocumentConfirmation.class);
		userContext.getDAOGroup().enhanceList(confirmationList, AccountingDocumentConfirmation.class);
		List<AccountingDocumentAuditing> auditingList = RetailscmBaseUtils.collectReferencedObjectWithType(userContext, list, AccountingDocumentAuditing.class);
		userContext.getDAOGroup().enhanceList(auditingList, AccountingDocumentAuditing.class);
		List<AccountingDocumentPosting> postingList = RetailscmBaseUtils.collectReferencedObjectWithType(userContext, list, AccountingDocumentPosting.class);
