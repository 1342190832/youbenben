...
public class SupplyOrderManagerImpl extends CustomRetailscmCheckerManager implements SupplyOrderManager, BusinessHandler{
 ...
 //--------------------------------------------------------------
	...
	protected SupplyOrder breakWithGoodsByPackaging(RetailscmUserContext userContext, String supplyOrderId, String packagingId,  String [] tokensExpr)
		 throws Exception{
	...
	*/
    ...
    protected void enhanceForListOfView(RetailscmUserContext userContext,SmartList<SupplyOrder> list) throws Exception {
		...
		List<SupplyOrderConfirmation> confirmationList = RetailscmBaseUtils.collectReferencedObjectWithType(userContext, list, SupplyOrderConfirmation.class);
		userContext.getDAOGroup().enhanceList(confirmationList, SupplyOrderConfirmation.class);
		List<SupplyOrderApproval> approvalList = RetailscmBaseUtils.collectReferencedObjectWithType(userContext, list, SupplyOrderApproval.class);
		userContext.getDAOGroup().enhanceList(approvalList, SupplyOrderApproval.class);
		List<SupplyOrderProcessing> processingList = RetailscmBaseUtils.collectReferencedObjectWithType(userContext, list, SupplyOrderProcessing.class);
		userContext.getDAOGroup().enhanceList(processingList, SupplyOrderProcessing.class);
		List<SupplyOrderPicking> pickingList = RetailscmBaseUtils.collectReferencedObjectWithType(userContext, list, SupplyOrderPicking.class);
		userContext.getDAOGroup().enhanceList(pickingList, SupplyOrderPicking.class);
		List<SupplyOrderShipment> shipmentList = RetailscmBaseUtils.collectReferencedObjectWithType(userContext, list, SupplyOrderShipment.class);
		userContext.getDAOGroup().enhanceList(shipmentList, SupplyOrderShipment.class);
		List<SupplyOrderDelivery> deliveryList = RetailscmBaseUtils.collectReferencedObjectWithType(userContext, list, SupplyOrderDelivery.class);
