...
public class UiActionJDBCTemplateDAO extends YoubenbenBaseDAOImpl implements UiActionDAO{
	...
	*/
	...
	protected boolean isUpdateRequest(UiAction uiAction){
 	...
 	protected Object[] prepareUiActionUpdateParameters(UiAction uiAction){
 		...
 		parameters[3] = uiAction.getBrief();
 		
 		
 		parameters[4] = uiAction.getImageUrl();
 		
 		
 		parameters[5] = uiAction.getLinkToUrl();
 		
 		
 		parameters[6] = uiAction.getExtraData();
 		 	
 		if(uiAction.getPage() != null){
 			parameters[7] = uiAction.getPage().getId();
 		}
 		
 		parameters[8] = uiAction.nextVersion();
 		parameters[9] = uiAction.getId();
 		parameters[10] = uiAction.getVersion();
