...
public class PeriodManagerImpl extends CustomHisCheckerManager implements PeriodManager, BusinessHandler{
	...
	public BlobObject exportExcelFromList(HisUserContext userContext, String id, String listName) throws Exception {
		...
		Map<String,Object> tokens = PeriodTokens.start().withTokenFromListName(listName).done();
		Period  period = (Period) this.loadPeriod(userContext, id, tokens);
		//to enrich reference object to let it show display name
		List<BaseEntity> entityListToNaming = period.collectRefercencesFromLists();
		periodDaoOf(userContext).alias(entityListToNaming);
		
		return exportListToExcel(userContext, period, listName);
		
	}
	@Override
	public BaseGridViewGenerator gridViewGenerator() {
		return new PeriodGridViewGenerator();
	}
