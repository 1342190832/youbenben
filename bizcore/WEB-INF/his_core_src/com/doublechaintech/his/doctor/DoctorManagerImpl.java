...
public class DoctorManagerImpl extends CustomHisCheckerManager implements DoctorManager, BusinessHandler{
	...
	public BlobObject exportExcelFromList(HisUserContext userContext, String id, String listName) throws Exception {
		...
		Map<String,Object> tokens = DoctorTokens.start().withTokenFromListName(listName).done();
		Doctor  doctor = (Doctor) this.loadDoctor(userContext, id, tokens);
		//to enrich reference object to let it show display name
		List<BaseEntity> entityListToNaming = doctor.collectRefercencesFromLists();
		doctorDaoOf(userContext).alias(entityListToNaming);
		
		return exportListToExcel(userContext, doctor, listName);
		
	}
	@Override
	public BaseGridViewGenerator gridViewGenerator() {
		return new DoctorGridViewGenerator();
	}
