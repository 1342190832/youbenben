...
public class DepartmentManagerImpl extends CustomHisCheckerManager implements DepartmentManager, BusinessHandler{
	...
	public BlobObject exportExcelFromList(HisUserContext userContext, String id, String listName) throws Exception {
		...
		Map<String,Object> tokens = DepartmentTokens.start().withTokenFromListName(listName).done();
		Department  department = (Department) this.loadDepartment(userContext, id, tokens);
		//to enrich reference object to let it show display name
		List<BaseEntity> entityListToNaming = department.collectRefercencesFromLists();
		departmentDaoOf(userContext).alias(entityListToNaming);
		
		return exportListToExcel(userContext, department, listName);
		
	}
	@Override
	public BaseGridViewGenerator gridViewGenerator() {
		return new DepartmentGridViewGenerator();
	}
