...
public class ExpenseTypeManagerImpl extends CustomHisCheckerManager implements ExpenseTypeManager, BusinessHandler{
	...
	public BlobObject exportExcelFromList(HisUserContext userContext, String id, String listName) throws Exception {
		...
		Map<String,Object> tokens = ExpenseTypeTokens.start().withTokenFromListName(listName).done();
		ExpenseType  expenseType = (ExpenseType) this.loadExpenseType(userContext, id, tokens);
		//to enrich reference object to let it show display name
		List<BaseEntity> entityListToNaming = expenseType.collectRefercencesFromLists();
		expenseTypeDaoOf(userContext).alias(entityListToNaming);
		
		return exportListToExcel(userContext, expenseType, listName);
		
	}
	@Override
	public BaseGridViewGenerator gridViewGenerator() {
		return new ExpenseTypeGridViewGenerator();
	}
