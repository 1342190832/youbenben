...
public class HospitalManagerImpl extends CustomHisCheckerManager implements HospitalManager, BusinessHandler{
	...
	public BlobObject exportExcelFromList(HisUserContext userContext, String id, String listName) throws Exception {
		...
		Map<String,Object> tokens = HospitalTokens.start().withTokenFromListName(listName).done();
		Hospital  hospital = (Hospital) this.loadHospital(userContext, id, tokens);
		//to enrich reference object to let it show display name
		List<BaseEntity> entityListToNaming = hospital.collectRefercencesFromLists();
		hospitalDaoOf(userContext).alias(entityListToNaming);
		
		return exportListToExcel(userContext, hospital, listName);
		
	}
	@Override
	public BaseGridViewGenerator gridViewGenerator() {
		return new HospitalGridViewGenerator();
	}
