...
public class CityPartnerJDBCTemplateDAO extends XingyouBaseDAOImpl implements CityPartnerDAO{
	...
	*/
	...
	protected boolean checkOptions(Map<String,Object> options, String optionToCheck){
 	...
 	protected boolean isExtractTownServiceCenterEnabled(Map<String,Object> options){
 		
	 	return checkOptions(options, CityPartnerTokens.TOWNSERVICECENTER);
 	}

 	protected boolean isSaveTownServiceCenterEnabled(Map<String,Object> options){
	...
	protected CityPartner loadInternalCityPartner(AccessKey accessKey, Map<String,Object> loadOptions) throws Exception{
		...
		CityPartner cityPartner = extractCityPartner(accessKey, loadOptions);
 		...
 		if(isExtractTownServiceCenterEnabled(loadOptions)){
	 		extractTownServiceCenter(cityPartner, loadOptions);
 		}
 	...
 	protected CityPartner extractTownServiceCenter(CityPartner cityPartner, Map<String,Object> options) throws Exception{

		if(cityPartner.getTownServiceCenter() == null){
			return cityPartner;
		}
		String townServiceCenterId = cityPartner.getTownServiceCenter().getId();
		if( townServiceCenterId == null){
			return cityPartner;
		}
		FranchisedOutletCityServiceCenter townServiceCenter = getFranchisedOutletCityServiceCenterDAO().load(townServiceCenterId,options);
		if(townServiceCenter != null){
	...
	protected CityPartner analyzePotentialCustomerContactList(CityPartner cityPartner, Map<String,Object> options){
 	...
 	public SmartList<CityPartner> findCityPartnerByTownServiceCenter(String franchisedOutletCityServiceCenterId,Map<String,Object> options){
 	
  		SmartList<CityPartner> resultList = queryWith(CityPartnerTable.COLUMN_TOWN_SERVICE_CENTER, franchisedOutletCityServiceCenterId, options, getCityPartnerMapper());
		// analyzeCityPartnerByTownServiceCenter(resultList, franchisedOutletCityServiceCenterId, options);
		return resultList;
 	}
 	 
 
 	public SmartList<CityPartner> findCityPartnerByTownServiceCenter(String franchisedOutletCityServiceCenterId, int start, int count,Map<String,Object> options){
 		
 		SmartList<CityPartner> resultList =  queryWithRange(CityPartnerTable.COLUMN_TOWN_SERVICE_CENTER, franchisedOutletCityServiceCenterId, options, getCityPartnerMapper(), start, count);
 		//analyzeCityPartnerByTownServiceCenter(resultList, franchisedOutletCityServiceCenterId, options);
 		return resultList;
 		
 	}
 	public void analyzeCityPartnerByTownServiceCenter(SmartList<CityPartner> resultList, String franchisedOutletCityServiceCenterId, Map<String,Object> options){
		if(resultList==null){
			return;//do nothing when the list is null.
		}
		
 		MultipleAccessKey filterKey = new MultipleAccessKey();
 		filterKey.put(CityPartner.TOWN_SERVICE_CENTER_PROPERTY, franchisedOutletCityServiceCenterId);
 	...
 	@Override
 	public int countCityPartnerByTownServiceCenter(String franchisedOutletCityServiceCenterId,Map<String,Object> options){

 		return countWith(CityPartnerTable.COLUMN_TOWN_SERVICE_CENTER, franchisedOutletCityServiceCenterId, options);
 	}
 	@Override
	public Map<String, Integer> countCityPartnerByTownServiceCenterIds(String[] ids, Map<String, Object> options) {
		return countWithIds(CityPartnerTable.COLUMN_TOWN_SERVICE_CENTER, ids, options);
	}
	...
	protected boolean isUpdateRequest(CityPartner cityPartner){
 	...
 	protected Object[] prepareCityPartnerUpdateParameters(CityPartner cityPartner){
 		...
 		if(cityPartner.getTownServiceCenter() != null){
 			parameters[2] = cityPartner.getTownServiceCenter().getId();
 		}
 	...
 	protected Object[] prepareCityPartnerCreateParameters(CityPartner cityPartner){
		...
		parameters[0] =  cityPartner.getId();
 		...
 		if(cityPartner.getTownServiceCenter() != null){
 			parameters[3] = cityPartner.getTownServiceCenter().getId();
	...
	protected CityPartner saveInternalCityPartner(CityPartner cityPartner, Map<String,Object> options){
		...
		saveCityPartner(cityPartner);
 		...
 		if(isSaveTownServiceCenterEnabled(options)){
	 		saveTownServiceCenter(cityPartner, options);
 		}
	...
	//======================================================================================
 	...
 	protected CityPartner saveTownServiceCenter(CityPartner cityPartner, Map<String,Object> options){
 		//Call inject DAO to execute this method
 		if(cityPartner.getTownServiceCenter() == null){
 			return cityPartner;//do nothing when it is null
 		}
 		
 		getFranchisedOutletCityServiceCenterDAO().save(cityPartner.getTownServiceCenter(),options);
 		return cityPartner;
    ...
    public SmartList<CityPartner> requestCandidateCityPartnerForPotentialCustomer(XingyouUserContext userContext, String ownerClass, String id, String filterKey, int pageNo, int pageSize) throws Exception {
        // NOTE: by default, ignore owner info, just return all by filter key.
		// You need override this method if you have different candidate-logic
		return findAllCandidateByFilter(CityPartnerTable.COLUMN_NAME, CityPartnerTable.COLUMN_TOWN_SERVICE_CENTER, filterKey, pageNo, pageSize, getCityPartnerMapper());
    }
		
    public SmartList<CityPartner> requestCandidateCityPartnerForPotentialCustomerContact(XingyouUserContext userContext, String ownerClass, String id, String filterKey, int pageNo, int pageSize) throws Exception {
        // NOTE: by default, ignore owner info, just return all by filter key.
		// You need override this method if you have different candidate-logic
		return findAllCandidateByFilter(CityPartnerTable.COLUMN_NAME, CityPartnerTable.COLUMN_TOWN_SERVICE_CENTER, filterKey, pageNo, pageSize, getCityPartnerMapper());
    }
	...
	public SmartList<PotentialCustomer> loadOurPotentialCustomerList(XingyouUserContext userContext, List<CityPartner> us, Map<String,Object> options) throws Exception{
	...
	public SmartList<PotentialCustomerContact> loadOurPotentialCustomerContactList(XingyouUserContext userContext, List<CityPartner> us, Map<String,Object> options) throws Exception{
		if (us == null || us.isEmpty()){
			return new SmartList<>();
		}
		Set<String> ids = us.stream().map(it->it.getId()).collect(Collectors.toSet());
